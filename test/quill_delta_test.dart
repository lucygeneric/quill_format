...
void main() {
  ...
  group('composeAttributes', () {
    ...
    test('missing', () {
      expect(Delta.composeAttributes(attributes, const {'i': true}), {'b': true, 'color': 'red', 'i': true});
    });

    test('overwrite', () {
      expect(Delta.composeAttributes(attributes, const {'b': false, 'color': 'blue'}), {'b': false, 'color': 'blue'});
    });

    test('remove', () {
      expect(Delta.composeAttributes(attributes, const {'b': null}), {'color': 'red'});
    });

    test('remove to null', () {
      expect(Delta.composeAttributes(attributes, const {'b': null, 'color': null}), isNull);
    });

    test('remove missing', () {
      expect(Delta.composeAttributes(attributes, const {'i': null}), attributes);
    });
  });
  ...
  group('transformAttributes', () {
    ...
    test('with priority', () {
      expect(Delta.transformAttributes(left, right, true), const {'italic': true});
    });
  ...
  group('Delta', () {
    ...
    test('json', () {
      final delta = Delta()..insert('abc', {'b': true})..insert('def')..insert({'a': 1});
      final result = json.encode(delta);
      expect(result, '[{"insert":"abc","attributes":{"b":true}},{"insert":"def"},{"insert":{"a":1}}]');
    ...
    group('push', () {
      ...
      test('consequent inserts (object) with different attributes do not merge', () {
        final delta = Delta()..insert(const {}, const {'b': true})..insert(const {});
        expect(delta.toList(), [
      ...
      test('consequent inserts (object) with same attributes do not merge', () {
        final delta = Delta()..insert(const {}, const {'b': true})..insert(const {}, const {'b': true});
        expect(delta.toList(), [
          Operation.insert(const {}, const {'b': true}),
          Operation.insert(const {}, const {'b': true}),
        ]);
      ...
      test('consequent inserts with same attributes merge', () {
        final ul = {'block': 'ul'};
        final doc = Delta()..insert('DartConf')..insert('\n', ul)..insert('Los Angeles')..insert('\n', ul);
        final change = Delta()
          ..retain(8)
          ..insert('\n', ul);
        final result = doc.compose(change);
        final expected = Delta()..insert('DartConf')..insert('\n\n', ul)..insert('Los Angeles')..insert('\n', ul);
      ...
      test('consequent deletes and inserts (object)', () {
        final doc = Delta()..insert(const {})..insert(const {})..insert(const {});
        final change = Delta()
          ..insert('YATA')
          ..delete(2)
          ..insert('YATA');
    ...
    group('compose', () {
      ...
      test('insert (object) and delete ordering', () {
        final a = Delta()..insert(const [1])..insert(const [2])..insert(const [3]);
        final b = Delta()..insert(const [1])..insert(const [2])..insert(const [3]);
        final insertFirst = Delta()
          ..retain(2)
          ..insert('X')
          ..delete(1);
        final deleteFirst = Delta()
          ...
          ..delete(1)
          ..insert('X');
        final expected = Delta()..insert(const [1])..insert(const [2])..insert('X');
        expect(a.compose(insertFirst), expected);
        expect(b.compose(deleteFirst), expected);
      });
    ...
    group('transform', () {
      ...
      test('insert + retain', () {
        ...
        var expected = Delta()..retain(1)..retain(1, const {'bold': true, 'color': 'red'});
      ...
      test('retain + retain without priority', () {
        ...
        var a2 = Delta()..retain(1, const {'color': 'blue'});
        var b2 = Delta()..retain(1, const {'bold': true, 'color': 'red'});
        var expected1 = Delta()..retain(1, const {'bold': true, 'color': 'red'});
    ...
    group('slice', () {
      ...
      test('start and end multiple chop', () {
        var slice = (Delta()..insert('0123', {'bold': true})..insert('4567')).slice(3, 5);
      ...
      test('split ops', () {
        var slice = (Delta()..insert('AB', {'bold': true})..insert('C')).slice(1, 2);
        var expected = Delta()..insert('B', {'bold': true});
        expect(slice, expected);
      });

      test('split ops multiple times', () {
        var slice = (Delta()..insert('ABC', {'bold': true})..insert('D')).slice(1, 2);
        var expected = Delta()..insert('B', {'bold': true});
        expect(slice, expected);
      });
    });
